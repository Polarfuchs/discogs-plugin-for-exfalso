...
class ResultComboBox(gtk.ComboBox):
    ...
    def __init__(self, model):
        ...
        def celldata(layout, cell, model, iter):
            ...
            if not entry:
                return
#            date = entry.getEarliestReleaseDate()
#            if date:
#                date = '%s, ' % date
#            else:
            date = ''
            entrytype = entry.__class__.__name__
            markup = ""
            
            if entrytype == "Release":
                markup = "Release <b>%s</b>" % (
                util.escape(entry.title))
                #util.escape(entry.artist.name),
                #date, entry.tracksCount)
            elif entrytype == "MasterRelease":
                markup = "MasterRelease <b>%s</b>" % (
                util.escape(entry.title))
                #util.escape(entry.artist.name),
                #date, entry.tracksCount)                
            elif entrytype == "Artist":
                markup = "Artist <b>%s</b>" % (
                util.escape(entry.name))
                #util.escape(entry.artist.name),
                #date, entry.tracksCount)
            elif entrytype == "Label":
                markup = "Label <b>%s</b>" % (
                util.escape(entry.data['name']))                
                #util.escape(entry.artist.name),
                #date, entry.tracksCount)  
            else:
...
class SearchWindow(gtk.Dialog):
    ...
    def __do_query(self, *args):
        ...
        self.discogs_search = discogs_client.Search(query=query)
        discogs_client.user_agent = 'discogs +http://github.com/polarfuchs'
        
        print "Hallo Welt"
        print self.discogs_search.numresults
        print self.discogs_search.exactresults
        print self.discogs_search.results()
        
        self._resultlist.clear()
#        if self.discogs_search.results() is None
#            self.result_label.set_markup("Error encountered. Please retry.")
#            self.search_button.set_sensitive(True)
#            return
        for release in self.discogs_search.results():
            self._resultlist.append((release, ))
#        if len(self.discogs_search.results()) > 0 and self.result_combo.get_active() == -1:
...
#                         self._query.getReleases, filt)
    ...
    def __init__(self, album, cache):
        ...
        super(SearchWindow, self).__init__("Discogs lookup", buttons=(
        ...
        sbtn = self.search_button = gtk.Button('S_uchen')
        sbtn.connect('clicked', self.__do_query)
        
        hb.pack_start(lbl, expand=False)
        hb.pack_start(sq)
        hb.pack_start(sbtn, expand=False)
...
class MyDiscogs(SongsMenuPlugin):
    PLUGIN_ID = "Discogs lookup"
    PLUGIN_NAME = "Discogs Lookup"
    PLUGIN_ICON = gtk.STOCK_CDROM
    PLUGIN_DESC = 'Retag an album based on a Discogs search.'
    PLUGIN_VERSION = '0.01'
